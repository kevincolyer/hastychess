...
func TestIsAttacked(t *testing.T) {
	...
	fmt.Println(&p)
	tap.Is(p.Side == WHITE, true, "White to play")
	tap.Is(IsAttacked(p.King[BLACK], WHITE, &p), false, "IsAttacked: Black King is in check (from white)")
	tap.Is(IsAttacked(p.King[BLACK], BLACK, &p), true, "IsAttacked: I as black am in check on the black kings square") // I as black am in check on the black kings square
	tap.Is(IsAttacked(p.King[WHITE], WHITE, &p), false, "IsAttacked: White King is NOT in check (from white)")
	tap.Is(IsAttacked(p.King[WHITE], BLACK, &p), true, "IsAttacked: White King is in check (from black)") // not what you might expect!

	// checking for theoretical check on empty square
	tap.Is(IsAttacked(E7, BLACK, &p), false, "IsAttacked: I as black am NOT in check on the square E7")
	tap.Is(IsAttacked(H5, BLACK, &p), true, "IsAttacked: I as black am  in check on the square H5")
	fmt.Println(&p)
}
